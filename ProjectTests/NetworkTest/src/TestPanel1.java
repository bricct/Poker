...
public class TestPanel1 extends JPanel {
	...
	public TestPanel1(Card card1, Card card2) {
	...
	public void paintComponent(Graphics g) {
		...
		BufferedImage back_reg = Sprite1.getBackSprite();
		if (this.card1 == null || this.card2 == null) {
			card1_reg = back_reg;
			card2_reg = back_reg;
		} else {
			card1_reg = Sprite1.getSprite(this.card1);
			card2_reg = Sprite1.getSprite(this.card2);
		}
		...
		for (int i = 0; i < 6; i++) {
			chip_spr[i] = Sprite1.getChipSprite(i).getScaledInstance(ch_size, ch_size, Image.SCALE_FAST);
		}
